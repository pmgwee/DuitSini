      ...
      {connected === null ? (
        ...
        <>
          ...
          <details className="mt-3 rounded-lg border border-border/40 bg-surface/30 px-3 py-2 text-[11px] text-muted-foreground/80">
            ...
            <p className="mt-1.5">
              Sharing key for a dedicated folder — mint it{" "}
              <b className="text-foreground">from that folder</b> and save it there too:
            </p>
            <ul className="mt-1.5 list-disc space-y-0.5 pl-4">
              <li>
                Windows:{" "}
                <code className="font-mono text-[10px]">
                  $env:CLAUDE_CONFIG_DIR=&quot;$env:USERPROFILE\.claude-pro&quot;; cmd /c claude setup-token
                </code>{" "}
                → copy the code →{" "}
                <code className="font-mono text-[10px]">
...
function ConnectedView({ sources, os }: { sources: ConnectedSource[]; os: OS }) {
  return (
    <div>
      ...
      {/* Upgrade path for members on an old sharer (or without a sharing key):
          they stop updating hours after their sign-in expires. Self-serve fix,
          shown without nagging the healthy ones. */}
      <details className="mt-3 rounded-lg border border-border/40 bg-surface/30 px-3 py-2 text-[11px] text-muted-foreground/80">
        <summary className="cursor-pointer select-none font-medium text-muted-foreground">
        ...
        <div className="mt-1.5 space-y-1 text-xs">
          <p>
            The sharer no longer auto-renews your Claude sign-in (renewing in a loop got accounts
...
/**
 * The one-time "sharing key" steps (the sharer never auto-refreshes sign-ins,
 * so this key is what keeps tracking alive around the clock). Rendered inside
 * the setup flows and the connected-view troubleshooter. Windows goes through
 * cmd /c: PowerShell's default execution policy blocks npm's claude.ps1 shim,
 * but cmd resolves claude.cmd/.exe and is exempt from the policy.
 */
function SharingKeySteps({ os }: { os: OS }) {
  return (
    <>
      ...
      <CmdLine text={os === "windows" ? "cmd /c claude setup-token" : "claude setup-token"} />
      <span className="block">
