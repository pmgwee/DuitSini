...
- **v4 — auto-refresh DISABLED (current behavior):** even v3's discipline still hit refresh-429 lockouts on dedicated login dirs, so `REFRESH_ENABLED = false` in the sharer template means the token endpoint is **never called**. The long-lived `claude setup-token` (→ `setup-token.txt` next to the creds, or `CLAUDE_SHARER_TOKEN`) is the intended steady-state auth; a plain sign-in works only while its on-disk access token is valid (the Claude CLI itself rotates it during normal use). An expired sign-in pauses that stream with a one-step fix message and re-checks disk every ~60s (setup-token drop-in or re-login resumes automatically). All v3 refresh machinery below is kept intact in the template — flip `REFRESH_ENABLED` to restore it.
