...
### Music recommendations (`lib/music/`)
Two-stage recommender modelled on how Spotify and Apple Music actually work: cheap candidate generation, then ranking. **Entirely anonymous** — there is no credential anywhere in this path, so nothing expires and nothing needs re-capturing.

- **History**: Google removed YT Music OAuth (Nov 2024); the cookie/SAPISID workaround that replaced it was then hardened and needed manual re-capture. Both are gone. `lib/google/ytmusic.ts`, `ytm-cookies.ts`, `/api/yt/cookies` and `cookie-connect.tsx` were **deleted** — don't reintroduce a credential here. (The `ytm_cookies` table still exists but is unused and unreferenced.)
- `sources.ts` — five signed-out InnerTube surfaces: song radio (`getUpNext`, 50/seed), radio continuation (raw `/next` + token — youtubei.js's `getUpNextContinuation()` helper throws in 17.2.0, so we call the endpoint directly), "You might also like", "Similar artists" (the stand-in for Spotify's removed `/related-artists`), and recommended playlists (the editorial layer). Radio queues are **deterministic per seed** (49–50/50 identical on repeat calls), so caching costs no variety — variety comes from rotating seeds, never from re-asking.
- `ranking.ts` — behavioural scoring (source trust × seed weight × positional discount, boosted by multi-source agreement), recency + skip penalties, per-artist cap, and an ε-greedy exploration budget (Spotify's BaRT). `pickSeeds` samples proportional to weight and forces one long-tail pick, because seeding off the top-N most-played regenerates the same neighbourhood — that was the original loop.
- `similarity.ts` — **the sequencing layer.** Spotify orders playlists by acoustic proximity; that API is gone and YouTube never exposed one. Do **not** substitute YouTube's own radio order: measured, `correlation(rank, similarity) = -0.145` and adjacent same-artist is 1/49, i.e. it is a *diversity-calibrated slate*, not a distance ordering (its native order scores worse than random shuffle on path cost). What works is co-occurrence: tracks appearing in the same radio queues are behaviourally close — the same signal Spotify's embeddings are trained on. Each candidate becomes a sparse vector over its sources; similarity is cosine; ordering is a greedy path. Measured: beats best-of-500 random shuffles, 0/39 adjacent same-artist.
- `store.ts` — `music_transitions` is the learned local-sequential model (Laplace-smoothed completion rate per `A→B` hand-off, damped by evidence, bounded ±0.5). This is what closes the gap left by the missing audio features, and it **converges with listening data** rather than being complete at deploy.
- **Signals**: `music_plays.skip_count` / `complete_count` plus `music_transitions` (migration `0018_music_signals.sql`). A skip inside 30s is the strongest negative signal both platforms use; the player emits it from `use-yt-player.ts`. Without this the ranker can only reshuffle history.
- **Likes & suppression** (migration `0019_music_likes.sql`, ADR-0006): `music_likes` and `music_suppressions`. Likes are **local** — writing back to YouTube needs `videos.rate` (`youtube.force-ssl` scope) and the app requests only `youtube.readonly`; widening it re-opens OAuth verification. Ranking uses `confidence()` in `ranking.ts` — **a like does not add a bonus, it multiplies a confidence term** (`c = 1 + α·r`, after Hu/Koren/Volinsky 2008): once a track is played, preference is already 1, and the like is what removes the ambiguity. Skips enter as *negative* evidence (measured: unknown `c=1.00`, played 3× `1.53`, +liked `2.93`, skipped 2× `0.05`, skipped-then-liked `1.11` — a like supersedes an old skip). Suppressed tracks are **dropped from the pool, not down-ranked**. `/api/yt/likes` (GET/POST/DELETE) and `/api/yt/suppress` (POST/DELETE; snooze = 30d, expiry needs no cleanup job).
...
- Provider: **OpenCode Go** (`https://opencode.ai/docs/go/`), model `grok-4.6`, reached through the OpenAI-compatible **Responses API** with `@ai-sdk/openai` + `ai`.
- Two Go-plan constraints are absorbed by the adapter, not the features: every request must carry an **`x-opencode-session`** header (missing → `400 MissingSessionID`; one id per server process, for routing/cache affinity), and **`grok-4.6` rejects `reasoning: "none"`** (400) — use `"low"` for a cheap pass. `LLM_MODEL` must be a model the plan serves (`GET {LLM_BASE_URL}/models`); an unserved id 401s and every caller degrades **silently**, so verify after changing it.
- Reasoning effort is **`xhigh`** at all three call sites, and the adapter sends **`forceReasoning: true`** alongside it. That flag is load-bearing: `@ai-sdk/openai` classifies reasoning models by matching *OpenAI's own* ids (o-series, gpt-5+), so for `grok-4.6` it silently **drops `reasoning` from the request body** and the provider falls back to `"high"`. Verified on the wire. The flag also sends the system prompt as the `developer` role and strips `temperature`, so the adapter omits temperature whenever reasoning is active.
- Reasoning tokens bill against `maxTokens` (~400–650 before any output), so budgets carry headroom — raising effort without raising the cap truncates the answer into a schema failure.
...
The phase pipeline is automated by three project skills in `.claude/skills/` (adapted from pinjun99/Sildenafil_coding, MIT — see `.claude/skills/README.md`):
